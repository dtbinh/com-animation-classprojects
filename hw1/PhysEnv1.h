...
public:
  ...
  void AddAttachedSpring(int v, float Ksh, float Ksd);							//加入一個連接到body的spring
  ...
  bool				m_IsInitialTime;															//	 Is initial time now (for verlet integration)
...
private:
  ...
  int				m_SpringCnt;		
  int				m_AttachPtsCnt;
  tParticle			*m_AttachedParticles;										//	The particles attached to body
  tSpring			*m_AttachedSprings;											//	The spring attached to body
  
// Operations
private:
  inline void	IntegrateSysOverTime(tParticle *initial,tParticle *source, tParticle *target, float deltaTime);
  inline void	AccIntegrateSysOverTime(tParticle *source, tParticle *target, tParticle *accuml, float deltaTime);	//	Sub-function for RK4Integrate()
  void	RK4Integrate( float DeltaTime);											//[Done] Integrator
  void	MidPointIntegrate( float DeltaTime);									//[Done] Integrator
  void	EulerIntegrate( float DeltaTime);										//Integrator
  void	VerletIntegrate ( float DeltaTime);										//[Done] Integrator
  void	ComputeForces( tParticle	*system );									//[Done] compute your force here
