...
float fDst1 = (L1-PV1).dotProduct( VNorm );
float fDst2 = (L2-PV1).dotProduct( VNorm );
...
if ( VTest.dotProduct( VIntersect-PV1 ) < 0.0f ) return false;
VTest = VNorm.crossProduct( PV3-PV2 );
if ( VTest.dotProduct( VIntersect-PV2 ) < 0.0f ) return false;
...
bool PointInFrustum( const Vector3 &P, Vector3 Normals[4], Vector3 Points[8] )
{
	for ( int x = 0; x < 4; x++ )
	{
		if ( Normals[x].dotProduct( P - Points[x*2] ) > 0 ) return false;
	}
    return true;
}
...
//
bool LineInFrustum( const Vector3 &LP1, const Vector3 &LP2, Vector3 Points[8] )
{
	Vector3 HitP;
...
bool TriInFrustum( Vector3 vTri[3], Vector3 Normals[4], Vector3 Points[8] )
{
 ...
 for( int x = 0; x < 4; x++ )
	...
	for ( i = 0; i < 3; i++ )
		{
			if ( Normals[x].dotProduct( vTri[i] - Points[x*2] ) < 0 ) break;
		}
	if ( i >= 3 ) return false;
	}
